...
import SearchFormTable from './searchfrommember.jsx';
...
export default class TableForm extends React.Component {
    ...
    onClose(selectedRows) {
        if (selectedRows) {
            var valueArray = [];
            valueArray = selectedRows.map(item => { 
                console.log('itema',item);
                if(this.props.label==="路线代码"){
                    return item.sroadcode;
                }else{
                    return item.usrname;
                }
    ...
    render() {
        ...
        const {btnShow,label,susertype,formData,field} = this.props;
        // console.log('zaizhei',formData)
        return (
            <FormItem
                    ...
                    <div>
                        ...
                        >
                            <div style={{ marginTop: '10px' }}>
                                {/* <CheckTable onClose={(e) => this.onClose(e)} btnShow = {btnShow} label={label}/> */}
                                <SearchFormTable onClose={(e) => this.onClose(e)} btnShow = {btnShow} label={label} susertype={susertype} LogoutModal={this.LogoutModal}/>
                            </div>
                        </Modal>
                    </div>
                )}
