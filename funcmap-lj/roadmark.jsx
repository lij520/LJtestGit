...
import { columns, landmark } from '@table/common-lj.js';
//引入接口
import { reqLandMarkList } from '../../../ajax-lj/index.jsx';
...
export default class RoadMarks extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pageNo: 1, //当前页码
  ...
  queryData = async (pageNo, pageSize, params) => {
    ...
    result = await reqLandMarkList(region_id.slice(0, 6), lower, Condition, queryValue, currentPage, pageSize);
    // console.log('result!!!!!!!!!!!!!!!', result)

    if (result.msg === "请求成功") {
      var data = [];
      result.data.glFacilitiesList.map((item, index) => {
        data.push({
          key: index + 1,
          ...item,
          keyno: item.keyno,
  ...
  onDetailTables = () => {
    ...
    w.location.href = `${url}#/detail/roadmarkdetail/${selectRow.keyno}`;
  }
      ...
      <div>
        ...
        <div>
          <div className="tributton">
            ...
            <Button type="primary" style={{ marginRight: '5px' }} onClick={this.listUpDown}>路标列表</Button>
          </div>
