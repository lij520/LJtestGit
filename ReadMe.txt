...
###day09
    ##1、redux理解
        什么是redux？redux是专门做状态管理的独立的第三方库，不是react插件，但一般都用在react项目
        作用？对应用中状态进行集中式的管理（写/读）
        开发：与react-redux，react-thunk等插件配合使用

    ##2、redux相关API
        redux中包含：creatStore(),applyModdleware(),combineReducers()
    ...
    ##3、redux核心概念（3个）
        1、action：默认对象(同步action)，{type:'xxx',data:value}，需要通过对应的actionCreator产生，
            它的值也可以是函数（异步action），需要引入redux-thunk才可以；
        2、reducer：根据旧的state和指定的action，返回一个新的state
            不能修改旧的state
        3、store：redux最核心的管理对象
            内部管理者：state和render
            提供方法：getState()，dispatch(actions)，subscribe(listener)
    ##4、redux工作流程：见test/js里面的图
...
###day10
    ...
    ##2、通过redux管理头部标题headTile数据
        1)reducer.js
        2)actions.js
        3)action-types.js
        4）相关组件：
            left-nav(index.jsx)：左侧导航栏
            header(index.jsx)：右侧头部
    ...
    ##4、自定义redux库
        ...
        3)combineReducers函数
            ...
            调用每个reducer函数得到对相应的新的state，并封装成一个新的总state对象返回
    ##5、自定义react-redux库
        1）react-redux向外暴露2个API:
            a、Provider
            b、connect函数
        2)Provider组件
            接收store属性
        ...
        3）connec函数
            接收2个参数：mapStateToProps(函数类型)和mapDispatchToProps(函数或对象类型)
