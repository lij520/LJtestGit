...
class Xmanagementroad extends Component {
    constructor(props){
      super(props);
      this.state = {
        beginTime: '2018-9-1', //ajax请求初始值
        start:moment('2019-9-1'), //DatePicker初始值
        endTime: '2019-12-30',
...
;
        ...
        myChart1.setOption({
            ...
            // color: ['#4e72b8','#ffce7b','#45b97c'],
            color: ['#2468a2','#33a3dc','#45b97c'],
            ...
            series: [
                ...
                //   label: { normal: { position: 'insideRight', show: true } }, //数值显示在柱状图top
                    //itemStyle: { color: },  //柱条颜色，默认从全局调色盘 option.color 获取颜色
                    data:[123,445,667,890], 
                },
                {
                    roadsUnderConstruction:'在建道路',
                    type: 'bar',
                    barGap: 0,
                    barWidth: '40%',
                    stack: 'topTen',
        ...
        var roadsDelayed = [];
        var roadsCompleted = [];
        var roadsUnderConstruction = [];       //数量数组（实际用来盛放Y坐标值）
        ...
        $.ajax({  
           ...
           // url:"http://192.168.0.137:8080/statistics/townRoadsConstructionInFo",
            data : formData,
            ...
            success : res => { 
              if(res.result == 1){
                ...
                $.each(res.statsList, function(index, item) {
                    if (item.regionName != null) {
                        regionName.push(item.regionName);//循环添加names
                        roadsCompleted.push(item.roadsCompleted);
                        roadsDelayed.push(item.roadsDelayed);
                        roadsUnderConstruction.push(item.roadsUnderConstruction);//循环添加nums
    ...
    render() {
        return (
            <div style={{height:'100%',width:'100%'}}>
              ...
              <div id="topTen" style={{width:'100%',height:'700%',marginTop:5}}></div>
...
export default Xmanagementroad;