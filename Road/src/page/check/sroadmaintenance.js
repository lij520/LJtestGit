...
class XRoadmaintenance extends Component {
    constructor(props){
      super(props);
      this.state = {
    ...
    setChartsData() {
        ...
        var myChart1 = echarts.init(document.getElementById('topTen3'));
        ...
        myChart1.setOption({
            ...
            series: [
                //前10名使用频次
                {
                    ...
                    //itemStyle: { color: },  //柱条颜  ，默认从全局调色盘 option.color 获取颜色
        ...
        $.ajax({  
            ...
            success : res => { 
              if(res.result == 1){
                ...
                $.each(res.patrol, function(index, item) {
                    if (item.regionName != null) {
                        regionName.push(item.regionName);//循环添加names
                        countPatrol.push(item.countPatrol);
       ...
       check() {
         $('<form  action="/excel/export" method="' + ('GET' || 'post') + '">' +  // action请求路径及推送方法
          ...
          '<input type="text" name="userId" value="' + _mm.getStorage('userInfo').userId + '"/>' + 
           ...
           '<input type="text" name="t" value="' +4 + '"/>' + 
           '</form>')
    ...
    render() {
        return (
            <div style={{height:'100%',width:'100%'}}>
              ...
              <div id="topTen3" style={{width:'100%',height:'700%',marginTop:5}}></div>
...
export default XRoadmaintenance;