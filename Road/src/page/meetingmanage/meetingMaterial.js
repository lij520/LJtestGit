 ...
 */
  ...
  downloadConferenceFile = (conferenceFileId) => {
    $('<form action="/conferenceFile/downloadConferenceFile" method="' + ('GET' || 'post') + '">' +  // action请求路径及推送方法
      '<input type="text" name="conferenceFileId" value="' + conferenceFileId + '"/>' + 
    ...
    /* var myThis=this;
    $.ajax({
      url: "http://192.168.3.171:8085/conferenceFile/downloadConferenceFile",
      type: "GET",
      async: false,
      data: {
  ...
  render() {
    ...
    return (
      <div>
       ...
       <div  className="meetLists"> <Table  columns={columns} dataSource={data} rowClassName={(record,index) => index % 2 === 0 ? "grayRow" : "whiteRow" }/></div>
        <br/>
        <Row>
